...
This can be done by creating a file named `.env.production` in the `client` project. This file is used in the production environment and
sets the SERVER_URL to the root context.

[source,javascript]
.client/.env.production
----
include::{sourceDir}/client/.env.production[indent=0]
----